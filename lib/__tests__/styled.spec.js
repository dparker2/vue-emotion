...
import ExistingClassesString from './ExistingClassesString.vue';
import ExistingClassesMix from './ExistingClassesMix.vue';
import ExistingClassesArray from './ExistingClassesArray.vue';
import ExistingClassesObject from './ExistingClassesObject.vue';
import TargetTest from './TargetTest.vue';
...
describe('styled', () => {
    ...
    test('merges emotion class with class list provided through any other means (attrs, etc)', () => {
      const cases = [
        ExistingClassesArray,
        ExistingClassesMix,
        ExistingClassesObject,
        ExistingClassesString
      ];
      ...
      for(let i = 0; i < length; i++) {
        ...
        expect(element.className.split(' ')).toEqual(expect.arrayContaining(['some-class', 'some-other-class']));
      }
    });
    ...
    test('styled components can be targetted', () => {
        const { element } = mount(TargetTest);
        const base = element.querySelector('#styled-base');
        expectStyle(base, 'color', 'red');  // Targetted
        expectStyle(base, 'display', 'flex');  // Not targetted
        expectStyle(base, 'font-size', '20px');  // Double targetted
    });
});
